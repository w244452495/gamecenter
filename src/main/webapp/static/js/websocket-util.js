function loadThreeUsersTable(data) {
	...
	for(var i = 0; i < data.length; i++) {
		tableHTML += "<table id=\"table-" + i + "\"><tr>";
		tableHTML += "<td><div id=\"SITE_" + formatIdTwoDigit(gameId) + formatIdTwoDigit(i + 1) + "00\" class=\"site-nobody\">进入</div></td>";
		tableHTML += "<td><div id=\"" + data[i].tableHtmlId + "\" class=\"table-three-users table-close\"></td>";
		tableHTML += "<td><div id=\"SITE_" + formatIdTwoDigit(gameId) + formatIdTwoDigit(i + 1) + "02\" class=\"site-nobody\">进入</div></td></tr>"; 
		tableHTML += "<tr><td></td>";
		tableHTML += "<td><div id=\"SITE_" + formatIdTwoDigit(gameId) + formatIdTwoDigit(i + 1) + "01\" class=\"site-nobody\">进入</div></td>";
...
function loadTableStatusAndSiteUser() {
	$.ajax({
		type: 'POST',
		url: 'table/loadTableStatusAndSiteUser',
		data: {
